# How to provision a new environment for shadowfax-demo
...
* Enter domains to secure (for example): `shadowfax-demo.herokuapp.com`
* Enter webroot path: `/home/deployer/apps/shadowfax_demo/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/shadowfax-demo.herokuapp.com/fullchain.pem /etc/ssl/shadowfax_demo.crt
ln -s /etc/letsencrypt/live/shadowfax-demo.herokuapp.com/privkey.pem /etc/ssl/shadowfax_demo.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/shadowfax_demo/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
11 6,18 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/shadowfax_demo/shared/public/
```
